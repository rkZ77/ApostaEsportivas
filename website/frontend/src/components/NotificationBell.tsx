...
function NotificationList({ onNavigate }: ListProps) {
  ...
  if (loading && items.length === 0) {
    return <p className="px-5 py-8 text-center text-sm text-zinc-500">Carregando...</p>
  }

  if (items.length === 0) {
    return (
      <div className="px-5 py-10 flex flex-col items-center text-center gap-2">
        <BellOff className="w-7 h-7 text-zinc-700" />
        <p className="text-sm text-zinc-500">Nenhuma notificação por aqui ainda.</p>
        ...
        {items.map(n => (
          <li key={n.id}>
            <button
              ...
              className={`w-full text-left px-4 py-3 flex gap-3 transition-colors hover:bg-zinc-800/50 ${
            ...
            >
              ...
              <span className="flex-1 min-w-0">
                <span className="flex items-baseline gap-2">
                  <span className={`text-sm leading-snug ${n.read ? 'text-zinc-300' : 'text-white font-bold'}`}>
                    {n.title}
                  </span>
                  <span className="text-[10px] text-zinc-600 ml-auto shrink-0">{timeAgo(n.created_at)}</span>
                </span>
                {n.body && (
                  <span className="block text-xs text-zinc-500 leading-snug mt-0.5 break-words">{n.body}</span>
                )}
              </span>
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute top-0.5 right-0.5 min-w-[16px] h-4 px-1 flex items-center justify-center rounded-full bg-green-500 text-[9px] font-black text-zinc-950 border border-zinc-950">
            {badge}
          </span>
        )}
      </button>
        ...
        {open && (isDesktop ? (
          <>
            ...
            <motion.div
              ...
              className="absolute right-0 top-full mt-2 w-[22rem] bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl z-50 overflow-hidden"
            >
              <div className="px-4 py-3 border-b border-zinc-800 flex items-center justify-between">
                <p className="text-white text-sm font-black">Notificações</p>
                <button onClick={() => setOpen(false)} aria-label="Fechar" className="text-zinc-500 hover:text-white transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </div>
          ...
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-0 left-0 right-0 z-[9999] bg-zinc-950 border-t border-zinc-800 rounded-t-2xl max-h-[85dvh] flex flex-col"
            >
              <div className="px-5 pt-4 pb-3 flex items-center justify-between shrink-0">
                <p className="text-white text-base font-black">Notificações</p>
                <button
                  onClick={() => setOpen(false)}
                  aria-label="Fechar"
                  className="w-8 h-8 flex items-center justify-center rounded-full border border-zinc-800 text-zinc-500 hover:text-white transition-colors"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
