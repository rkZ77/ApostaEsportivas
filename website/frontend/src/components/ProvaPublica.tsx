...
export default function ProvaPublica({ compacta = false }: {
  ...
  const tiles = [
    ...
    { rotulo: 'Picks resolvidos', valor: resumo ? String(resumo.total) : null },
    { rotulo: 'Lucro', valor: resumo ? fmtUnits(resumo.profit) : null, destaque: true },
    ...(compacta ? [] : [{ rotulo: 'ROI', valor: resumo ? `${resumo.roi.toFixed(1)}%` : null }]),
  ]
  ...
  return (
    <div className="bg-surface-1 border border-line rounded-lg p-5">
      <div className="flex items-center justify-between gap-3 mb-4">
        ...
        {!compacta && (
          <Link to="/resultados" className="text-[11px] text-accent-ink font-semibold inline-flex items-center gap-1 shrink-0">
