  ...
  if (notFound || !pick) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center p-6 text-center">
        <p className="text-zinc-400 text-lg font-semibold mb-2">Pick não encontrado</p>
        <p className="text-zinc-600 text-sm mb-6">O link pode ter expirado ou sido removido.</p>
    ...
    <div className="min-h-screen bg-black flex flex-col">
      ...
      <div className="px-4 pt-5 pb-3 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-1.5">
          <span className="text-white font-black text-lg tracking-tight">Pick<span className="text-green-400">IA</span></span>
        </Link>
        {user ? (
          <Link to="/picks" className="text-xs text-zinc-400 hover:text-white transition-colors">Ver meus picks</Link>
        ) : (
      ...
      <div className="flex-1 px-4 py-4 max-w-lg mx-auto w-full">
        <div className="bg-zinc-950 border border-zinc-800 rounded-2xl overflow-hidden">
            ...
            <div className="flex items-center gap-2">
              ...
              {pick.league_name && (
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-[10px] text-zinc-600 truncate max-w-[90px]">{pick.league_name}</span>
                </div>
              )}
            </div>
            {rs ? (
          ...
          {isVip && fullPick ? (
            <div className="px-5 py-4 space-y-2 border-b border-zinc-800/60">
              ...
              {fullPick.reasoning && (
                <p className="text-xs text-zinc-400 leading-relaxed line-clamp-4">{fullPick.reasoning}</p>
              )}
            </div>
          ) : (
            <div className="px-5 py-4 border-b border-zinc-800/60">
              <div className="flex items-center gap-3 px-4 py-3 bg-zinc-900 border border-zinc-800 rounded-xl">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-zinc-500 mt-0.5">Disponível apenas para assinantes VIP</p>
                </div>
              </div>
        ...
        {/* Footer branding */}
        <p className="text-center text-[10px] text-zinc-700 mt-6">Pick IA · Tips por Inteligência Artificial</p>
      </div>
    </div>
  )
}
