        ...
        <div className="bg-surface-0 border border-line rounded-lg overflow-hidden">
          ...
          {isVip && fullPick ? (
            <div className="px-5 py-4 space-y-2 border-b border-line/60">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-ink-1">{rotuloDoMercado(fullPick.market, fullPick.line)}</span>
              </div>
              {fullPick.reasoning && (
          ...
          {!isVip && (
            <div className="px-5 py-5 space-y-3">
              ...
              <Link
                to={signupUrl}
                className="flex items-center justify-center gap-2 w-full py-3.5 rounded-md bg-green-600 hover:bg-green-500 text-on-fill font-black text-sm transition-colors"
              >
                <TrendingUp className="w-4 h-4" />
                {user ? 'Assinar' : 'Criar conta gratuita e assinar'}
              </Link>
              {!user && (
