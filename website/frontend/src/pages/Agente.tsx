  ...
  if (!isVip && !isAdmin) {
    return (
      ...
      >
        <EmptyState
          Icon={Lock}
          title="Agente exclusivo do Pro"
          description="O Agente IA analisa picks, banca, jogos ao vivo e responde qualquer dúvida em tempo real. Disponível no plano Pick IA Pro."
      ...
      <div className={`bg-surface-0 border-t border-line ${cookieBannerVisible ? 'pb-24 sm:pb-14' : ''}`}>
        <div className="max-w-3xl mx-auto px-4 py-3 flex gap-2 items-end">
          ...
          <button
            ...
            className="w-12 h-12 rounded-md bg-green-600 hover:bg-green-500 disabled:opacity-40 disabled:cursor-not-allowed transition-colors flex items-center justify-center text-on-fill shrink-0"
          >
            {loading ? (
